...
def get_gemini_response(user_prompt, my_lat=None, my_lon=None):
    try:
        ...
        # AGGIORNATO: Caricamento 3 file (Multi-File RAG)
        with open("itinerary.json", "r", encoding="utf-8") as f: itinerary_json = json.load(f)
        with open("itinerary_text.txt", "r", encoding="utf-8") as f: base_text = f.read()
        with open("klm_compendio.txt", "r", encoding="utf-8") as f: klm_text = f.read()
